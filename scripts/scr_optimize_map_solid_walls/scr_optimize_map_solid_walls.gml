...
while (inst != noone) //repeat
{
    ...
    if (instance_position(x + (image_xscale * CELLSIZE) + CELLSIZE, y, o_solid) == noone) {
        inst = noone; //if last than break ( checking last because of vertical collision)
        break;
    } else {
        inst = instance_position(x + (image_xscale * CELLSIZE), y, o_solid) //find new neighbor
    }
}
...
if (image_xscale == 1) { //if our xscale doesnot change.
    ...
    var inst = instance_place(x, y + (image_yscale * CELLSIZE), o_solid);
    ...
    while (inst != noone) //repeat
    {
        ...
        inst = instance_position(x, y + (image_yscale * CELLSIZE), o_solid) //check for another neighbor
    }
}